...
import { getPurchasedByUserId, UnsaveItinerary } from "@/lib/actions/itinerary.actions"
...
export default function PurchasedPage() { 
  ...
  const [saves, setPurchases] = useState<SavedItinerary[] | null>(null)
  const [filteredPurchases, setFilteredPurchases] = useState<SavedItinerary[] | null>(null)
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event: string, session: Session | null) => {
      ...
      setUser(currentUser)
      if (currentUser) {
        const userItineraries = await getPurchasedByUserId(currentUser.id)
        setPurchases(userItineraries as SavedItinerary[])
        setFilteredPurchases(userItineraries as SavedItinerary[])
        setLoading(false)
  ...
  const refreshItineraries = async () => {
    if (user) {
      const userItineraries = await getPurchasedByUserId(user.id)
      setPurchases(userItineraries as SavedItinerary[])
      setFilteredPurchases(userItineraries as SavedItinerary[])
    }
  }
  ...
  const handleSearch = (searchTerm: string) => {
    if (searchTerm === "") {
      setFilteredPurchases(saves)
      return
    }

    setSearchTerm(searchTerm)
    setFilteredPurchases(saves?.filter(itinerary => 
  ...
  const handleUnsave = async (itineraryId: string) => {
    ...
    if (result.success) {
      ...
      setPurchases(saves?.filter(itinerary => itinerary.id !== itineraryId))
      setFilteredPurchases(filteredPurchases?.filter(itinerary => itinerary.id !== itineraryId))
      refreshItineraries()
    }
    else {
    ...
    <div className="min-h-screen bg-white py-12 sm:pt-[6rem]">
      <div className="container mx-auto px-6 sm:px-12 md:px-[3rem] lg:px-[6rem]">
        <div className="flex flex-row items-center gap-2 mb-10">
          <h1 className="text-3xl font-semibold">Purchased Itineraries</h1>
        ...
        {filteredPurchases && filteredPurchases.length > 0 ? (
            ...
            {filteredPurchases.map((itinerary) => (
              ...
              >
                ...
                <div className="absolute top-2 right-2">
                  <BookmarkElement itineraryId={itinerary.id} currentUserId={user?.id || ''} color="white" onUnsave={handleUnsave} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center py-12 px-4 rounded-xl border-2 border-dashed">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-1">No Purchased Itineraries Yet</h3>
            <p className="text-gray-600 mb-4">Start exploring itineraries you like (coming soon)</p>
            <button 
              disabled={true}
